...
from .vit_adaptive_prompt.vit import AdaptivePromptedVisionTransformer
from .vit_adaptive_prompt.vit_moco import vit_base as adaptive_prompt_vit_base
from .vit_adaptive_prompt.vit_mae import build_model as adaptive_prompt_mae_vit_model
...
def build_mae_model(
    ...
    if prompt_cfg is not None:
        if prompt_cfg.ADAPTIVE:
            model = adaptive_prompt_mae_vit_model(model_type, prompt_cfg)
        else:
...
def build_mocov3_model(
    ...
    if prompt_cfg is not None:
        if prompt_cfg.ADAPTIVE:
            model = adaptive_prompt_vit_base(prompt_cfg)
        else:
...
def build_vit_sup_models(
    ...
    if prompt_cfg is not None:
        if prompt_cfg.ADAPTIVE:
            model = AdaptivePromptedVisionTransformer(
