...
class ViT(nn.Module):
    ...
    def build_backbone(self, prompt_cfg, cfg, adapter_cfg, load_pretrain, vis):
        ...
        self.enc, self.feat_dim = build_vit_sup_models(
            cfg.DATA.FEATURE, cfg.DATA.CROPSIZE, prompt_cfg, cfg.MODEL.MODEL_ROOT, adapter_cfg, load_pretrain, True
        )
    ...
    def forward(self, x, return_feature=False, is_train=False):
        ...
        if self.cfg.VIS:
            # x, cls_tokens = self.enc(x)  # batch_size x self.feat_dim  # TODO
            x, attn_maps = self.enc(x, vis=True, is_train=is_train)  # batch_size x self.feat_dim  # TODO
        else:
            x = self.enc(x)
